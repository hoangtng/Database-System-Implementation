...
int Sorted::GetNextByBinarySearch(Record &fetchme, CNF &cnf, Record &literal)
{ // Notice: file may be empty.
...
MetaInfo DBFile::readMetaInfo(string &meta_file)
{
    ...
    {
        cerr << "[Error] In DBFile::strToMetaInfo(fstream* meta): Invalid file type '" << file_type_str << "', meta file: " << meta_file << "'" << endl;
        return MetaInfo{};
    }
    meta.close();
    return MetaInfo{
...
int DBFile::Open(const char *fpath)
{
    ...
    {
        cerr << "[Error] In DBFile::Open (const char *f_path): Invalid file type '" << m_metaInfo.fileType << "', meta filepath: '" << m_metaPath << "'" << endl;
        return 0;
    }
...
void Heap::Add(Record &addme)
{
    ...
    { // The last data page is full.
        Page new_page;
        new_page.Append(&addme);
        m_file->file->AddPage(&new_page, last_data_page_idx + 1);
    }
    else
    { // The last data page is not full and add record successfully, then rewrite that page in file
        if (last_data_page_idx >= 0)
        { // last page is from the file
            m_file->file->AddPage(&last_data_page, last_data_page_idx);
        }
        else
        { // last page is newly created
...
int Heap::GetNext(Record &fetchme)
{
    ...
    if (status == 2)
    { // Has arrived at the last record in current page.
        if (m_file->currentDataPageIdx + 1 > last_data_page_idx)
        { // Current page is the last page in current file.
#ifdef verbose
