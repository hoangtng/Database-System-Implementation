...
void QueryPlan::optimizePredicate()
{
    ...
    std::cout << "Optimal predicate (cost " << min_cost << "): " << Util::ParseTreeToString(optimal_predicate) << "\n"
              << std::endl;
}
...
void SelectNode::execute(const std::map<int, Pipe *> &pipes)
{
    ...
    {
        // To be completed
        // You need to follow the explanation of execute function in A_final.pdf,
        // create the corresponding operator,
        // and run it.
        // Remember to assign the operator to the member variable executor.
...
ProjectNode::ProjectNode(NameList *atts, QueryPlanNode *child)
{
    ...
    {
        // To be completed
        // Iterate each attribute in atts,
        // and store their information in keepMe (which is the parameter you need for Project),
        // as well as in keep_atts (which is an array to store the name and datatype of each attribute in keepMe).
        // You need to find out each name and datatype from atts and save to keep_atts.
        int index = input_schema->Find(atts->name);
        if (index == -1)
        {
            throw runtime_error("[Error] Attribute not found");
        }

        keepMe[numAttsOutput] = index;                              // fill here by the information you found from atts
        keep_atts[numAttsOutput].name = atts->name;                 // fill here by the information you found from atts
        keep_atts[numAttsOutput].myType = input_atts[index].myType; // fill here by the information you found from atts
        numAttsOutput++;
        atts = atts->next;
    }
...
GroupByNode::GroupByNode(FuncOperator *agg_func, NameList *group_att_names, QueryPlanNode *child)
{
    ...
    std::cout << "GROUPBY input sch: " << left_sch->toString() << std::endl;
