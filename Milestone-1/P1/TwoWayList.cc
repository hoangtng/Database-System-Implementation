...
TwoWayList <Type> :: TwoWayList ()
{
	...
	// Notice: Now this is an empty list. So list->first and list->last are both nodes without data.
	// list->first and list->last are only used for determining starting and ending of the list.
...
{
	// list->current is NOT pointing to current record, but the previous node of it.
	// When there is an empty list, list->current will be pointing to list->first. 
	// So here we will remove list->current->next instead of list->current.
	Node *temp = list->current->next;
