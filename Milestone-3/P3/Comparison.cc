...
int OrderMaker::GetAtts(Attribute *atts, int *att_indices)
{
	...
	for (int i = 0; i < numAtts; i++)
	{
		...
		case Int:
			...
			break;
		case Double:
			atts[i] = Attribute("Double", Double);
			break;
		case String:
			atts[i] = Attribute("String", String);
			break;
		}
...
int CNF::findAtt(int att)
{
	...
	for (int i = 0; i < numAnds; i++)
	{
		for (int j = 0; j < orLens[i]; j++)
		{
			flat_idx++;
			if (
				att == orList[i][j].whichAtt1 && orList[i][j].operand1 != Literal || att == orList[i][j].whichAtt2 && orList[i][j].operand2 != Literal)
			{
